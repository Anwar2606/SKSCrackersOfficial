...
const ProductList = () => {
  ...
  const padSno = (sno) => {
    // Ensure `sno` is a string, or provide a fallback if it's missing
    const snoString = sno ? String(sno) : "000";
  
    // Example: pad the serial number with leading zeros
    return snoString.replace(/^\d+$/, (match) => match.padStart(3, '0'));
  };
...
const downloadPDF = () => {
  ...
  Object.keys(groupedProducts).forEach((category) => {
    ...
    groupedProducts[category].forEach((product) => {
      const productData = [
        padSno(product.sno), // Make sure to pad and handle S.No properly
        product.name,
        `Rs.${product.regularprice.toFixed(2)}`,
        `Rs.${product.saleprice.toFixed(2)}`,
        product.category
